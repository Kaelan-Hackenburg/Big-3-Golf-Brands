...
December_Tweets = pd.read_csv('December_Covid_Tweets.csv')
...
December_Tweets = December_Tweets.drop(columns=['Tweet_Count'])
#print(December_Tweets.head(10))

# Pull the 'Tweet_Text' column and place in a variable to aid in text mining process
December_Tweet_Text = December_Tweets['Tweet_Text']
#print(December_Tweet_Text.head(10))

# Place tweet text into an array
December_Sentences = []
for word in December_Tweet_Text:
    December_Sentences.append(word)

#print(December_Sentences)
...
lines = []
for line in December_Sentences:
    words = line.split()
    for w in words:
        lines.append(w)
...
# plt.title('Organizations Discussed in December 2020')
# plt.ylabel('Word from Tweet', fontsize = 12)
# plt.xlabel('Word Count', fontsize = 12)
# plt.show()
...
# plt.figure(figsize = (10, 5))
# sea.barplot(df.values, df.index, alpha = 0.8)
# plt.title('Top People Discussed in December 2020')
