...
list_remove <- c("xf0","xe2","x9f","x80", "x99s",
                 "t.co", "https", "covid","vaccine","pfizer")
...
# Apply the above regex
results5$word <- str_remove_all(results5$word, pattern2)
results5$word[results5$word == ""] <- NA
################################## Wordcloud ##################################
# Wordcloud will be created using the 'results5' dataframe
set.seed(1234)
wordcloud(words = results5$word, freq = results5$n, min.freq = 1.0, max.words = 250,
          random.order = FALSE, rot.per = 0.35, colors = brewer.pal(9, "Set1"))
###############################################################################
